...
var Field = /** @class */ (function (_super) {
    ...
    Field.prototype.render = function () {
        invariant_1(false, this.constructor.name + " elements are for schema configuration only and should not be rendered");
...
var Flexbox = unwrapExports(dist);
 ...
 */
var FormEditStates = keymirror({
    ALWAYS: null,
    NEVER: null,
    SELECTED: null,
    TABLE: null
});
var FormGroupLayout = keymirror({
    COLUMN: null,
    INLINE: null,
    ROW: null
});
...
function getFieldsFromSchema(schema) {
    var fields = {};
    if (React__default.isValidElement(schema) && isChildSchema(schema)) {
        var props = schema.props;
        React__default.Children.forEach(props.children, function (field) {
...
function getRulesFromSchema(schema) {
    var rules = {};
    if (React__default.isValidElement(schema) && isChildSchema(schema)) {
        React__default.Children.forEach(schema.props.children, function (child) {
            if (isChildField(child)) {
                var required = child.props.required || false;
                var validation = deepcopy(child.props.validation);
...
var Form = /** @class */ (function (_super) {
     ...
     *   - edit selection
     */
    Form.prototype.getFieldProps = function (_a, fieldName) {
        var _this = this;
        var formFields = _a.formFields, formRules = _a.formRules, formHiddenList = _a.formHiddenList;
        var _b = this.props, labelWidth = _b.labelWidth, edit = _b.edit, value = _b.value, initialValue = _b.initialValue;
        var selection = this.state.selection;
        var props = {};
        props.labelWidth = labelWidth || 300;
        if (lodash.has(formFields, fieldName)) {
            props.key = fieldName;
            ...
            if (lodash.includes(formHiddenList, fieldName)) {
                props.disabled = true;
                props.hidden = true;
            }
        }
        ...
        if (lodash.has(formRules, fieldName)) {
        ...
        // Callbacks
        props.onSelectItem = function (fieldName) { return _this.handleSelectItem(fieldName); };
        props.onErrorCountChange = function (fieldName, count) {
            return _this.handleErrorCountChange(fieldName, count);
        };
        props.onMissingCountChange = function (fieldName, count) {
            return _this.handleMissingCountChange(fieldName, count);
        };
        props.onChange = function (fieldName, d) { return _this.handleChange(fieldName, d); };
        props.onBlur = function () { return _this.handleBlur(); };
        return props;
    };
    /**
     ...
     */
    Form.prototype.queueChange = function () {
        var _this = this;
        var name = this.props.name;
        if (!this._deferSet) {
            lodash.defer(function () {
                _this._deferSet = false;
                // Write in missingCounts and errorCounts into our state
                var state = {};
                if (_this._pendingMissing) {
                    state.missingCounts = _this._pendingMissing;
                }
                if (_this._pendingErrors) {
                    state.errorCounts = _this._pendingErrors;
                }
                ...
                var ignoreList = _this.getHiddenFields(formFields);
                // Missing count callback
                if (_this._pendingMissing) {
                    var missingFields_1 = [];
                    lodash.each(_this._pendingMissing, function (c, fieldName) {
                        if (!lodash.includes(ignoreList, fieldName)) {
                            missingCount += c;
                            missingFields_1.push(fieldName);
                        }
                    });
                    if (_this.props.onMissingCountChange) {
                        _this.props.onMissingCountChange(name, missingCount, missingFields_1);
                    }
                    _this._pendingMissing = null;
                }
                // Error callback
                if (_this._pendingErrors) {
                    var errorFields_1 = [];
                    lodash.each(_this._pendingErrors, function (c, fieldName) {
                        if (!lodash.includes(ignoreList, fieldName)) {
                            missingCount += c;
                            errorFields_1.push(fieldName);
                        }
                        errorCount += c;
                    });
                    if (_this.props.onErrorCountChange) {
                        _this.props.onErrorCountChange(_this.props.name, errorCount, errorFields_1);
                    }
                    _this._pendingErrors = null;
                }
                // On change callback
                if (_this._pendingValues) {
                    if (_this.props.onChange) {
                        _this.props.onChange(_this.props.name, _this._pendingValues);
                    }
                    _this._pendingValues = null;
                }
            });
            this._deferSet = true;
        }
     ...
     */
    Form.prototype.handleErrorCountChange = function (fieldName, errorCount) {
     ...
     */
    Form.prototype.handleMissingCountChange = function (fieldName, missingCount) {
     ...
     */
    Form.prototype.handleChange = function (fieldName, newValue) {
        var onPendingChange = this.props.onPendingChange;
    ...
    Form.prototype.handleBlur = function () {
     ...
     */
    Form.prototype.handleSelectItem = function (fieldName) {
     ...
     */
    Form.prototype.getHiddenFields = function (formFields) {
        var visible = this.props.visible;
        var result = [];
        if (visible) {
            lodash.each(formFields, function (field, fieldName) {
                var makeHidden;
                var tags = field.tags || [];
                if (lodash.isArray(visible)) {
                    makeHidden = !(lodash.intersection(tags, visible).length > 0 || lodash.includes(tags, "all"));
                }
                else {
                    makeHidden = !(lodash.includes(tags, visible) || lodash.includes(tags, "all"));
                }
                if (makeHidden) {
     ...
     * track state.
     */
    Form.prototype.traverseChildren = function (formStruct, childList) {
        var _this = this;
        var childCount = React__default.Children.count(childList);
        var children = [];
        React__default.Children.forEach(childList, function (child, i) {
            if (child) {
                var newChild = void 0;
                ...
                if (typeof child.props.children !== "string") {
                    if (lodash.has(child.props, "field")) {
                        ...
                        props = __assign(__assign({}, props), _this.getFieldProps(formStruct, fieldName));
                    }
                    if (React__default.Children.count(child.props.children) > 0) {
                        props = __assign(__assign({}, props), { children: _this.traverseChildren(formStruct, child.props.children) });
                    }
                }
    ...
    Form.prototype.shouldComponentUpdate = function (nextProps, nextState) {
        var _a = this.props, value = _a.value, edit = _a.edit, schema = _a.schema, visibility = _a.visibility;
        var selection = this.state.selection;
        var update = nextProps.value !== value ||
            nextProps.edit !== edit ||
    ...
    Form.prototype.render = function () {
        var _this = this;
        var _a = this.props, edit = _a.edit, formStyle = _a.formStyle, formClassName = _a.formClassName, formKey = _a.formKey, inline = _a.inline, inner = _a.inner, schema = _a.schema;
        var formFields = getFieldsFromSchema(schema);
        ...
        if (edit === FormEditStates.TABLE) {
            return (React__default.createElement(Flexbox, { flexDirection: "row", className: formClassName, key: formKey }, this.traverseChildren(s, this.props.children)));
        }
        else {
            if (inner) {
                return (React__default.createElement("form", { className: formClass, style: formStyle, key: formKey, onSubmit: function (e) {
                        _this.handleSubmit(e);
                    }, noValidate: true }, this.traverseChildren(s, this.props.children)));
            }
            else {
                return (React__default.createElement("div", { className: formClassName, style: formStyle, key: formKey }, this.traverseChildren(s, this.props.children)));
            }
        }
    };
    return Form;
...
function inlineDoneButtonStyle(marginLeft, enabled) {
    var isEnabled = lodash.isUndefined(enabled) ? true : enabled;
    return {
        padding: 5,
...
var List = /** @class */ (function (_super) {
    ...
    List.prototype.handleMouseEnter = function () {
        this.setState({ hover: true });
    };
    List.prototype.handleMouseLeave = function () {
    ...
    List.prototype.revertItem = function (index) {
        if (this.props.onRevertItem) {
            this.props.onRevertItem(index);
            this.selectItem(null);
        }
    };
    List.prototype.handleDeselect = function () {
        this.selectItem(null);
    };
    List.prototype.render = function () {
        ...
        if (addPlus && mouseOver) {
            plusIcon = (React__default.createElement("i", { key: "plus", className: "glyphicon glyphicon-plus icon add-action", onClick: function () { return _this.addItem(); } }));
        }
        else {
            plusIcon = React__default.createElement("div", null);
        }
        var LISTWIDTH = 600;
        var ICONWIDTH = 28;
        // Build the item list, which is a list of table rows, each row containing
        // an item and a [-] icon used for removing that item.
        var itemList = lodash.map(this.props.items, function (item, index) {
            ...
            var edit;
            var isEditable;
            if (_this.props.hideEditRemove) {
                isEditable = _this.props.hideEditRemove && index === _this.props.items.length - 1;
            }
            else {
                isEditable = true;
            }
            if (isEditable) {
                if (addMinus && !itemMinusHide && mouseOver) {
                    minus = (React__default.createElement("i", { id: "" + index, key: minusActionKey, className: "glyphicon glyphicon-remove hostile_icon delete-action" }));
                }
                else {
                    listEditItemClass += " no-controls";
                    minus = React__default.createElement("div", { className: "icon delete-action" });
                }
                // Edit item icon
                if (addEdit && mouseOver) {
                    edit = (React__default.createElement("i", { id: "" + index, key: minusActionKey, style: { paddingLeft: 5, paddingRight: 5 }, className: "glyphicon glyphicon-pencil icon edit-action active" }));
                }
            }
            var minusAction = addMinus ? (React__default.createElement(Flexbox, { width: "28px" },
                React__default.createElement("div", { onClick: function () { return _this.removeItem(index); } },
                    React__default.createElement("span", { key: actionSpanKey, className: "icon", style: { paddingLeft: 5, paddingRight: 5 } }, minus)))) : (React__default.createElement("div", { style: { height: 30 } }));
            var editAction = addEdit ? (React__default.createElement(Flexbox, { width: "28px" },
                React__default.createElement("div", { onClick: function () {
                        _this.selectItem(index);
                    } },
                    React__default.createElement("span", { key: actionSpanKey, className: "icon", style: { verticalAlign: "top" } }, edit)))) : (React__default.createElement("div", null));
            // JSX for each row, includes: UI Item and [x] remove item button
            if (!isBeingEdited) {
                return (React__default.createElement("li", { 
                    // height="80px"
                    // width="600px"
                    key: itemKey, className: "esnet-forms-list-item", style: {
                        borderBottomStyle: "solid",
                        borderBottomColor: "#DDD",
                        borderBottomWidth: 1
                    } },
                    React__default.createElement(Flexbox, { flexDirection: "row", style: { width: "100%", paddingTop: 5 } },
                        React__default.createElement(Flexbox, { style: { width: LISTWIDTH - ICONWIDTH * 2 } },
                            React__default.createElement("span", { key: itemSpanKey, className: listEditItemClass }, item)),
                        minusAction,
                        editAction)));
            }
            else {
                return (React__default.createElement("li", { key: itemKey, className: "esnet-forms-list-item", style: {
                        borderBottomStyle: "solid",
                        borderBottomColor: "#DDD",
                        borderBottomWidth: 1
                    } },
                    React__default.createElement(Flexbox, { flexDirection: "row", style: { width: "100%", paddingTop: 10, paddingBottom: 10 } },
                        React__default.createElement(Flexbox, { flexDirection: "column" },
                            React__default.createElement(Flexbox, { style: { width: LISTWIDTH - ICONWIDTH * 2 } },
                                React__default.createElement("span", { key: itemSpanKey, className: listEditItemClass }, item)),
                            React__default.createElement(Flexbox, { style: {
                                    fontSize: 12,
                                    marginLeft: _this.props.buttonIndent
                                } },
                                _this.props.canCommitItem ? (React__default.createElement("span", { style: inlineDoneButtonStyle(0, true), onClick: function () { return _this.handleDeselect(); } }, "DONE")) : (React__default.createElement("span", { style: inlineDoneButtonStyle(0, false) }, "DONE")),
                                React__default.createElement("span", { style: inlineCancelButtonStyle(), onClick: function () { return _this.revertItem(index); } }, "CANCEL"))),
                        minusAction)));
            }
        });
        // Build the [+] elements
        ...
        if (addPlus && mouseOver) {
            if (this.props.plusElement) {
                ...
                plus = (React__default.createElement(Flexbox, { flexDirection: "row" },
                    React__default.createElement(Flexbox, { width: "28px" },
                        React__default.createElement("span", { key: "plus", className: "icon", style: { verticalAlign: "top", fontSize: 10 } }, plusIcon)),
                    React__default.createElement(Flexbox, { width: "28px" })));
            }
        }
        else {
            plus = React__default.createElement("div", { style: { height: 35 } });
        }
        //
        // Build the header
        //
        ...
        return (React__default.createElement("div", { style: {
                ...
                borderTopWidth: 1,
                borderTopColor: "#DDD"
            }, onMouseEnter: function () { return _this.handleMouseEnter(); }, onMouseLeave: function () { return _this.handleMouseLeave(); } },
            React__default.createElement("ul", { className: "esnet-forms-listeditview-container" }, itemList),
            plus));
    };
    return List;
}(React__default.Component));
...
var revalidator_1 = revalidator.validate;
...
function editAction(enabled, handler) {
    ...
    var action = React__default.createElement("span", null);
    if (enabled) {
        action = (React__default.createElement("span", { style: { marginLeft: 5 }, onClick: handler },
            React__default.createElement("i", { style: iconStyle, className: "glyphicon glyphicon-pencil icon edit-action active" })));
    }
    else {
        action = React__default.createElement("div", null);
    }
    return action;
}
...
var Collection = function Collection(value) {
  return isCollection$1(value) ? value : Seq(value);
};
...
var SetCollection = /*@__PURE__*/(function (Collection) {
  function SetCollection(value) {
    return isCollection$1(value) && !isAssociative(value) ? value : SetSeq(value);
  }
...
function isImmutable(maybeImmutable) {
  return isCollection$1(maybeImmutable) || isRecord(maybeImmutable);
}
...
var SetSeq = /*@__PURE__*/(function (Seq) {
  function SetSeq(value) {
    return (isCollection$1(value) && !isAssociative(value)
      ? value
      : IndexedSeq(value)
    ).toSetSeq();
  }
...
Collection.isIterable = isCollection$1;
...
function formGroup(Control) {
    var wrapped = /** @class */ (function (_super) {
        ...
        function Group(props) {
            var _this = _super.call(this, props) || this;
            _this.state = { over: false, error: false };
            return _this;
        }
        Group.prototype.render = function () {
            var _a = this.props, _b = _a.hidden, hidden = _b === void 0 ? false : _b, width = _a.width, props = __rest(_a, ["hidden", "width"]);
            var name = props.name, label = props.label, key = props.key, edit = props.edit, 
            // disabled,
            required = props.required, showRequired = props.showRequired, onSelectItem = props.onSelectItem, value = props.value, initialValue = props.initialValue, layout = props.layout, labelWidth = props.labelWidth;
            var isBeingEdited = edit;
            var hasChanged = lodash.isNull(initialValue) ? false : !Immutable.is(value, initialValue);
            var selectStyle = {};
            //
            // Hidden
            //
            ...
            var control = (React__default.createElement("div", { style: {
                    width: controlWidth
                } },
                React__default.createElement(Control, __assign({}, props, { onEditItem: onSelectItem }))));
            //
            // Required
            //
            var requiredMarker;
            if (required && showRequired) {
                requiredMarker = (React__default.createElement("span", { className: "group-required", style: { paddingLeft: 3 } }, "*"));
            }
            else {
                requiredMarker = React__default.createElement("span", null);
            }
            //
            // Label
            //
            ...
            var fieldLabel = (React__default.createElement("div", { className: labelClasses, style: {
                    color: labelColor,
                    marginLeft: marginLeft,
                    paddingTop: 3,
                    whiteSpace: "nowrap"
            ...
            var labelSize = labelWidth ? labelWidth + "px" : "300px";
            // Group
            switch (this.props.layout) {
                case FormGroupLayout.INLINE:
                    return (React__default.createElement(Flexbox, { flexDirection: "column", width: "" + controlWidth }, control));
                case FormGroupLayout.COLUMN:
                    return (React__default.createElement(Flexbox, { flexDirection: "column" },
                        React__default.createElement(Flexbox, { flexDirection: "row" },
                            React__default.createElement(Flexbox, null, fieldLabel),
                            React__default.createElement(Flexbox, { minWidth: "14px", width: "14px" }, requiredMarker)),
                        React__default.createElement(Flexbox, { flexDirection: "row" },
                            React__default.createElement(Flexbox, { flexGrow: 1, style: selectStyle }, control))));
                case FormGroupLayout.ROW:
                    return (React__default.createElement(Flexbox, { flexDirection: "row" },
                        React__default.createElement(Flexbox, { minWidth: labelSize, width: labelSize }, fieldLabel),
                        React__default.createElement(Flexbox, { minWidth: "14px", width: "14px" }, requiredMarker),
                        React__default.createElement(Flexbox, { width: "" + controlWidth, style: selectStyle },
                            React__default.createElement("div", { onDoubleClick: function () {
                                    return onSelectItem && !isBeingEdited ? onSelectItem(name) : null;
                                } }, control)),
                        React__default.createElement(Flexbox, { flexGrow: 1 })));
                default:
                    return React__default.createElement("div", null);
            }
        };
...
function textView(value) {
    return React__default.createElement("span", { style: { minHeight: 28 } }, value);
}
...
var TextEdit = /** @class */ (function (_super) {
    ...
    TextEdit.prototype.handleMouseEnter = function () {
        this.setState({ hover: true });
    };
    TextEdit.prototype.handleMouseLeave = function () {
        this.setState({ hover: false });
    };
    TextEdit.prototype.handleEditItem = function () {
        if (this.props.onEditItem) {
            this.props.onEditItem(this.props.name);
        }
    };
    TextEdit.prototype.isEmpty = function (value) {
        return lodash.isNull(value) || lodash.isUndefined(value) || value === "";
    };
    TextEdit.prototype.isMissing = function (value) {
        return this.props.required && !this.props.disabled && this.isEmpty(value);
    };
    TextEdit.prototype.getError = function (value) {
        var result = {
        ...
        if (obj && rules) {
            var validation = revalidator_1(obj, rules, { cast: true });
            var name_1 = this.props.name || "Value";
            var msg = void 0;
            if (!validation.valid) {
                msg = name_1 + " " + validation.errors[0].message;
    ...
    TextEdit.prototype.componentWillReceiveProps = function (nextProps) {
        ...
        if (this.state.value !== nextProps.value && !this.state.isFocused) {
            ...
            var error = this.getError(nextProps.value).validationError;
    ...
    TextEdit.prototype.componentDidMount = function () {
        ...
        var validationError = this.getError(this.props.value).validationError;
    ...
    TextEdit.prototype.handleChange = function (e) {
        var _this = this;
        var name = this.props.name;
        var value = e.target.value;
        this.setState({ value: value }, function () {
            var missing = _this.props.required && _this.isEmpty(value);
            var validationError = _this.getError(value).validationError;
            var cast = value;
            // Callbacks
            if (_this.props.onErrorCountChange) {
                _this.props.onErrorCountChange(name, validationError ? 1 : 0);
            }
            if (_this.props.onMissingCountChange) {
                _this.props.onMissingCountChange(name, missing ? 1 : 0);
            }
            if (_this.props.onChange) {
                if (lodash.has(_this.props.rules, "type")) {
                    switch (_this.props.rules.type) {
                        case "integer":
                ...
                _this.props.onChange(name, cast);
            }
        });
    };
    TextEdit.prototype.handleFocus = function () {
        if (!this.state.isFocused) {
            this.setState({ isFocused: true, oldValue: this.props.value });
        }
    };
    TextEdit.prototype.handleKeyPress = function (e) {
        if (e.key === "Enter") {
            if (!e.shiftKey) {
                this.handleDone();
            }
    ...
    TextEdit.prototype.handleDone = function () {
        if (this.props.onBlur) {
            this.props.onBlur(this.props.name);
        }
        this.setState({ isFocused: false, hover: false, oldValue: null });
    };
    TextEdit.prototype.handleCancel = function () {
        if (this.props.onChange) {
            var v = this.state.oldValue;
            var cast = v;
            if (lodash.has(this.props.rules, "type")) {
    ...
    TextEdit.prototype.render = function () {
        ...
        var _a = this.getError(this.props.value), validationError = _a.validationError, validationErrorMessage = _a.validationErrorMessage;
        if (this.props.edit) {
            // Error style/message
            var className = "";
            ...
            // Warning style
            var style = isMissing ? { background: colors.MISSING_COLOR_BG } : {};
            var type = this.props.type || "text";
            var canCommit = !isMissing && !validationError;
            return (React__default.createElement(Flexbox, { flexDirection: "row", style: { width: "100%" } },
                React__default.createElement("div", { className: className, style: { width: "100%" } },
                    React__default.createElement("input", { ref: function (input) {
                            _this.textInput = input;
                        }, className: "form-control input-sm", style: style, type: type, disabled: this.props.disabled, placeholder: this.props.placeholder, value: this.state.value ? this.state.value : "", onChange: function (e) { return _this.handleChange(e); }, onFocus: function () { return _this.handleFocus(); }, onKeyUp: function (e) { return _this.handleKeyPress(e); } }),
                    React__default.createElement("div", { className: helpClassName }, msg)),
                this.props.selected ? (React__default.createElement("span", { style: { marginTop: 3 } },
                    canCommit ? (React__default.createElement("span", { style: inlineDoneButtonStyle(5, true), onClick: function () { return _this.handleDone(); } }, "DONE")) : (React__default.createElement("span", { style: inlineDoneButtonStyle(5, false) }, "DONE")),
                    React__default.createElement("span", { style: inlineCancelButtonStyle(), onClick: function () { return _this.handleCancel(); } }, "CANCEL"))) : (React__default.createElement("div", null))));
        }
        else {
            var view = this.props.view || textView;
            var text = isMissing ? (React__default.createElement("span", null)) : (React__default.createElement("span", { style: { minHeight: 28 } }, view(this.props.value)));
            var edit = editAction(this.state.hover && this.props.allowEdit, function () {
                return _this.handleEditItem();
            });
            var style = inlineStyle(validationError, isMissing);
            return (React__default.createElement("div", { style: style, key: "key-" + isMissing + "-" + validationError, onMouseEnter: function () { return _this.handleMouseEnter(); }, onMouseLeave: function () { return _this.handleMouseLeave(); } },
                text,
                edit));
        }
    };
...
var TextEdit$1 = formGroup(TextEdit);
...
var View = /** @class */ (function (_super) {
    ...
    View.prototype.render = function () {
        ...
        if (!view) {
            ...
            return React__default.createElement("div", { style: style }, view(value));
        }
    };
    return View;
}(React__default.Component));
var View$1 = formGroup(View);
 ...
 */
function formList(ItemComponent, hideEditRemove, actionButtonIndex) {
    return /** @class */ (function (_super) {
        ...
        function ListManager(props) {
            ...
            _this.missing = [];
            // Initial state
            _this.state = { selected: null, oldValue: null };
            return _this;
        }
        // Handle a new item being selected
        ListManager.prototype.handleSelectItem = function (i) {
        ...
        ListManager.prototype.handleRevertItem = function (i) {
        ...
        ListManager.prototype.handleChangeItem = function (i, value) {
        ...
        ListManager.prototype.handleMissingCountChange = function (i, missingCount) {
        ...
        ListManager.prototype.handleErrorCountChange = function (i, errorCount) {
        ...
        ListManager.prototype.handleRemovedItem = function (i) {
            var value = this.props.value;
            var n = 1;
        ...
        ListManager.prototype.handleAddItem = function () {
            var value = this.props.value;
            this.errors.push(0);
            this.missing.push(0);
            // Callbacks
        ...
        ListManager.prototype.numMissing = function () {
            var total = 0;
            lodash.each(this.missing, function (c) {
                total += c;
            });
            return total;
        };
        // Utility function to determine the total number of errors in the entire list
        ListManager.prototype.numErrors = function () {
            var total = 0;
            lodash.each(this.errors, function (c) {
                total += c;
            });
            return total;
        };
        ListManager.prototype.render = function () {
            ...
            this.props.value.forEach(function (itemValue, index) {
                // Each item is initialized from the initialValues prop
                var itemInitialValue = _this.props.initialValue
                    ? _this.props.initialValue.get(index)
                    : null;
                var itemProps = {
                    key: "" + index,
                    name: "item-" + index,
                    innerForm: true,
                    hideMinus: hideEditRemove && index < _this.props.value.size - 1,
                    types: _this.props.types,
                    value: itemValue,
                    initialValue: itemInitialValue,
                    editable: _this.props.edit,
                    edit: index === selected,
                    onErrorCountChange: function (index, errorCount) {
                        return _this.handleErrorCountChange(index, errorCount);
                    },
                    onMissingCountChange: function (index, missingCount) {
                        return _this.handleMissingCountChange(index, missingCount);
                    },
                    onChange: function (index, value) { return _this.handleChangeItem(index, value); }
                };
                var element = React__default.createElement(ItemComponent, itemProps);
                itemComponents.push(element);
            });
            var hasErrors = lodash.find(this.errors, function (item) { return item >= 1; });
            var hasMissing = lodash.find(this.missing, function (item) { return item >= 1; });
            var plusElement = (hasErrors || hasMissing) && hideEditRemove ? React__default.createElement("div", null) : null;
            var _a = this.props, _b = _a.canAddItems, canAddItems = _b === void 0 ? true : _b, _c = _a.canRemoveItems, canRemoveItems = _c === void 0 ? true : _c;
            var canCommitItem = !lodash.isNull(selected)
                ? this.errors[selected] === 0 && this.missing[selected] === 0
                : false;
            return (React__default.createElement(List, { items: itemComponents, buttonIndent: actionButtonIndex, canAddItems: canAddItems, canRemoveItems: canRemoveItems, canEditItems: true, hideEditRemove: hideEditRemove, plusWidth: 400, plusElement: plusElement, canCommitItem: canCommitItem, onAddItem: function () { return _this.handleAddItem(); }, onRemoveItem: function (index) { return _this.handleRemovedItem(index); }, onSelectItem: function (index) { return _this.handleSelectItem(index); }, onRevertItem: function (index) { return _this.handleRevertItem(index); } }));
        };
        return ListManager;
    }(React__default.Component));
}
...
exports.Field = Field;
exports.Form = Form;
exports.FormEditStates = FormEditStates;
exports.FormGroupLayout = FormGroupLayout;
exports.List = List;
exports.MyComponent = MyComponent;
exports.Schema = Schema;
exports.TextEdit = TextEdit$1;
exports.View = View$1;
