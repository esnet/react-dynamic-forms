...
export default {
  /* FormMixin */
  propTypes: { schema: React.PropTypes.object.isRequired },
  getInitialState() {
    ...
    const initialValues = deepCopy(this.props.values);

    // Setup formValues
    let values = {};
    _.each(attrs, (attr, attrName) => {
      const defaultValue = _.has(attr, "defaultValue")
        ? attr["defaultValue"]
        : undefined;
      if (initialValues) {
        const v = _.has(initialValues, attrName)
          ...
          : defaultValue;
        values[attrName] = { value: v, initialValue: deepCopy(v) };
      } else {
        values[attrName] = {
          value: defaultValue,
          initialValue: deepCopy(defaultValue)
        };
      }
    });
    ...
    if (this.getInitialVisibility) {
      ...
      if (tag) {
        _.each(attrs, (attr, attrName) => {
          ...
          if (_.isArray(tag)) {
            makeHidden = !(_.intersection(tags, tag).length > 0 ||
              _.contains(tags, "all"));
          } else {
  ...
  getAttr(attrName) {
    ...
    const formAttrs = deepCopy(this.state.formAttrs);
    const formRules = deepCopy(this.state.formRules);
    const formValues = deepCopy(this.state.formValues);
    const formHiddenList = deepCopy(this.state.formHiddenList);
  ...
  initialValue(attrName) {
    ...
    if (!_.has(formValues, attrName)) {
      throw new Error(
        `Requested initialValue for attr '${attrName}' could not be found`
      );
    }
    return deepCopy(formValues[attrName].initialValue);
  },
  value(attrName) {
    const formValues = this.state.formValues;
    if (!_.has(formValues, attrName)) {
      throw new Error(
        `Requested value for attr '${attrName}' could not be found`
      );
    }
    return deepCopy(formValues[attrName].value);
  },
  /**
  ...
  setValue(key, value) {
    ...
    this._pendingFormValues = this._pendingFormValues ||
      deepCopy(this.state.formValues);
    if (!_.has(this._pendingFormValues, key)) {
      throw new Error(
        `Tried to set value on form, but key '${key}' doesn't exist`
      );
    }
  ...
  setVisibility(tag) {
    ...
    this._pendingHiddenList = this._pendingHiddenList ||
      deepCopy(this.state.formHiddenList);
